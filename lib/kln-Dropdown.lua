...
Widget that presents a list of predefined options for selection
when clicked.

New:
  parent      - (frame) Frame to attach dropdown to.
  label       - (string) Textual label of the dropdown.
  tooltipText - (string) Descriptive hover text. Optional.
  items       - (table) Textual option/value pairs to populate
                dropdown with.
  keepShownOnClick - (boolean) Menu is not hidden when clicked.

GetValue: Enables the dropdown and returns the current value.

SetValue: Sets the current value of the dropdown.
  value - (mixed) The value to set.
  text  - (string) Label of the value. Optional. If not given
          and the value already exists in the dropdown, that
          value's text is used. Otherwise the value is used
          at the label.

GetList: Returns the list of values associated with the dropdown.

SetList: Sets the list of values for the dropdown.
  list - (table) Textual option/value pairs to populate
         dropdown with.

Disable: Makes the dropdown grayed and unclickable.

Enable: Makes the dropdown normal colored and clickable.
--]]

local MINOR_VERSION = 20180810
local MAX_LIST_SIZE = 15

local lib, oldminor = LibStub:NewLibrary("kln-Dropdown", MINOR_VERSION)
if not lib then return end

lib.listFrames = lib.listFrames or {}   -- Object pool

---------------------------------------------------------------------

local CreateList -- Implemented far below
...
function CreateList(dropdown)
  ...
  local list = CreateFrame("Button", "klnConfigDropdown" .. id, dropdown)
  ...
  lib.listFrames[#lib.listFrames] = list
  dropdown.list = list
  return list
end
...
function lib:New(parent, label, tooltipText, items, keepShownOnClick)
  assert(type(parent) == "table" and type(rawget(parent, 0)) == "userdata", "kln-Dropdown: parent must be a frame")
